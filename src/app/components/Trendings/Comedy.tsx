...
export interface ComedyMovie {
  page: number;
  results: ComedyResult[];
  total_pages: number;
  total_results: number;
}

export interface ComedyResult {
  adult: boolean;
...
const Comedy = () => {
  ...
  const { data, isLoading } = useQuery(["comedyMovieData"], async () => {
    const { data } = await axios.get(
      "https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&with_genres=35",
      options
    );
    return data as ComedyMovie;
  });
                    ...
                    <SwiperSlide key={`slide-${index}`}>
                      <div className="flex my-2 justify-between items-center gap-2">
                        ...
                        <div className="flex justify-center items-center">
                          <div>
                            <AiOutlineHeart className="text-2xl text-white hover:text-white duration-300" />
                          </div>
                          <div>
